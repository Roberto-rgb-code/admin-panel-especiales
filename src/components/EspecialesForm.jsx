// admin-panel-especiales/src/components/EspecialesForm.jsx
...
const EspecialesForm = () => {
  ...
  const fetchEspecial = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${apiUrl}/api/especiales-destacados/${id}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (id) {
        await axios.put(`${apiUrl}/api/especiales-destacados/${id}`, formData, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
      } else {
        await axios.post(`${apiUrl}/api/especiales-destacados`, formData, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
      }
      navigate('/');
  ...
  return (
    <div className="form-container">
      <h1>{id ? 'Editar Especial Destacado' : 'Agregar Especial Destacado'}</h1>
      {loading ? (
        <p>Cargando...</p>
      ) : error ? (
